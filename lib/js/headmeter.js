...
class DoomHead {
    ...
    _getIndex(percents) {
        ...
        let index = Math.ceil(percents / 20);
        return this._getNumInRange(index, 1, 10);
    }
    ...
    _getPercents(currentPercents, newDead = 0, newAlive = 0, oldDead = 0, oldAlive = 0) {
        ...
        const voteMultiplier = 5;
        percents -= (newDead - oldDead)/voteMultiplier;
        percents += (newAlive - oldAlive)/voteMultiplier;
        return percents >= currentPercents ? Math.trunc(percents) : Math.ceil(percents);
    }
}
